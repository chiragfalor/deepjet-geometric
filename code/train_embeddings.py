...
def train():
    ...
    for data in train_loader:
        ...
        for i in range(BATCHSIZE):
            ...
            # truth_batch to int
            truth_batch = truth_batch.to(dtype=torch.int64, device=device)
            vtx_indices = (data.x_vtx_batch == i)
            vtx_enc_batch = vtx_enc[vtx_indices, :]
            # calculate the loss due to the pfc embedding
            ...
            # raise(Exception("stop"))
            pfc_loss = euclidean_loss(pfc_enc_batch[0, :], true_pfc_encoding[0, :])
        ...
        if counter % 100 == 0:
            print("Iteration: ", counter, " Loss: ", total_loss.item())
