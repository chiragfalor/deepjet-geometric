...
def visualize_embeddings(pfc_embeddings,pfc_truth, save_path):
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        truth = data.y.view(1, -1)
        out, batch, pfc_embeddings = net(data.x_pfc, data.x_pfc_batch)
        # visualize the embeddings
        visualize_embeddings(pfc_embeddings.cpu().numpy(), truth, 'vis_emb{}.png'.format(epoch_num))
        # distinguish neutral and charged embeddings
        distinguish_neutral_charged_embeddings(pfc_embeddings.cpu().numpy(), truth, 'vis_nc_emb{}.png'.format(epoch_num), data.x_pfc)
